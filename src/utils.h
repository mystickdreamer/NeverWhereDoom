...
#define GET_COMBATEXP(ch)	((ch)->combatexpertise)
...
#define RIDDEN_BY(ch)		((ch)->ridden_by)
...
#define CAN_SEE_IN_DARK(ch) \
   (AFF_FLAGGED(ch, AFF_INFRAVISION) || (!IS_NPC(ch) && PRF_FLAGGED(ch, PRF_HOLYLIGHT)) \
    || (AFF_FLAGGED(ch, AFF_LOW_LIGHT) && OUTSIDE(ch)))
...
#define IS_SORCERER(ch)         (GET_CLASS_RANKS(ch, CLASS_SORCERER) > 0)
#define IS_DRUID(ch)            (GET_CLASS_RANKS(ch, CLASS_DRUID) > 0)
#define IS_BARD(ch)             (GET_CLASS_RANKS(ch, CLASS_BARD) > 0)
#define IS_RANGER(ch)           (GET_CLASS_RANKS(ch, CLASS_RANGER) > 0)
#define IS_BARBARIAN(ch)        (GET_CLASS_RANKS(ch, CLASS_BARBARIAN) > 0)
#define IS_ARCANE_ARCHER(ch)    (GET_CLASS_RANKS(ch, CLASS_ARCANE_ARCHER) > 0)
#define IS_ARCANE_TRICKSTER(ch) (GET_CLASS_RANKS(ch, CLASS_ARCANE_TRICKSTER) > 0)
#define IS_ARCHMAGE(ch)         (GET_CLASS_RANKS(ch, CLASS_ARCHMAGE) > 0)
