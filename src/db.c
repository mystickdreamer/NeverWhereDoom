...
CVSHEADER("$CVSHeader: cwg/rasputin/src/db.c,v 1.27 2005/05/07 01:13:00 zizazat Exp $");
...
void mob_autobalance(struct char_data *ch)
{
    ...
    GET_HIT(ch) = GET_HITDICE(ch);
    GET_MANA(ch) = class_hit_die_size[GET_CLASS(ch)];
    GET_MOVE(ch) = GET_HITDICE(ch) * 2;
    GET_EXP(ch) = level_exp(GET_HITDICE(ch) + 1) / (GET_HITDICE(ch)*20);
    GET_ARMOR(ch) = (GET_HITDICE(ch)/2);
    GET_NDD(ch) = MAX(1, GET_HITDICE(ch)/10);
    GET_SDD(ch) = 4 + MAX(1, GET_HITDICE(ch)/10);
    GET_ACCURACY_MOD(ch) = GET_HITDICE(ch)/5;
    GET_DAMAGE_MOD(ch) = GET_HITDICE(ch)/3;
}
...
int parse_simple_mob(FILE *mob_f, struct char_data *ch, int nr)
{
  ...
  if (GET_LEVEL(ch) < 1) {
    log("SYSERR: 0 HD Mob (#%d) detected!",  nr);
    GET_HITDICE(ch) = 1;
  }
...
char *parse_object(FILE *obj_f, int nr)
{
  ...
  /* Convert old CWG-SunTzu style armor values to CWG-Rasputin. Should no longer be needed I think.
    if (GET_OBJ_TYPE(obj_proto + i) == ITEM_ARMOR) {
      GET_OBJ_VAL(obj_proto + i, 0) = GET_OBJ_VAL(obj_proto + i, 0) / 10;
        if(bitsavetodisk) {
          add_to_save_list(zone_table[real_zone_by_thing(nr)].number, 1);
          converting = TRUE;
        }
  ...
  for (;;) {
	...
	exit(1);
      ...
      if (t[0] >= APPLY_UNUSED0 && t[0] <= APPLY_UNUSED4) {
...
void add_unique_id(struct obj_data *obj)
{
  ...
  if (!obj_unique_hash_pools)
    init_obj_unique_hash();
  if (obj->unique_id == -1) {
   if (sizeof(cl_uint64) > sizeof(cl_uint32))
     obj->unique_id = (((cl_uint64)circle_random()) << 32) + circle_random();
    else
