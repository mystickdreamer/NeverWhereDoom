...
ACMD(do_value)
{
  ...
  if (!*arg) {
    ...
    case SCMD_COMBATEXP:
        if (GET_COMBATEXP(ch)){
          send_to_char(ch, "Your current combat expertise level is -%d accuracy +%ddodge bonus.\r\n",
                        GET_COMBATEXP(ch), GET_COMBATEXP(ch));
          return;
         } else {
          send_to_char(ch, "You are currently not using your combat expertise to your advantage.\r\n");
          return;
        }           
      break;  
    }
  ...
  if (isdigit(*arg)) {
    ...
    case SCMD_COMBATEXP:
      if ((value_lev = atoi(arg)) != 0) {
        if (value_lev < 0)
          send_to_char(ch, "Heh, heh, heh.. we are jolly funny today, eh?\r\n");
        else if (value_lev > GET_BAB(ch))
          send_to_char(ch, "You may only specify a value up to %d.\r\n", GET_BAB(ch));
        else {
	  send_to_char(ch, "Okay, you'll sacrifice %d points of accuracy for dodge bonus to AC.\r\n", value_lev);
	  GET_COMBATEXP(ch) = value_lev;
        }
      } else {
        send_to_char(ch, "Okay, you will no longer use your combat expertise.\r\n");
        GET_COMBATEXP(ch) = 0;
      }
      break;  
    default:
  ...
  case SCMD_AUTOASSIST:
   ...
   case SCMD_SCREENWIDTH:
     if (!*arg)
       ...
       GET_SCREEN_WIDTH(ch) = MIN(MAX(atoi(arg), 40), 200);
...
ACMD(do_file)
{
  ...
  struct file_struct {
    ...
    char *file;
     ...
     { "none",           ADMLVL_GRGOD,    "Does Nothing" },
     { "bug",            ADMLVL_GRGOD,    "../lib/misc/bugs"},
     { "typo",           ADMLVL_GRGOD,   "../lib/misc/typos"},
     { "ideas",          ADMLVL_GRGOD,    "../lib/misc/ideas"},
     { "xnames",         ADMLVL_GRGOD,     "../lib/misc/xnames"},
     { "levels",         ADMLVL_GRGOD,    "../log/levels" },
     { "rip",            ADMLVL_GRGOD,    "../log/rip" },
     { "players",        ADMLVL_GRGOD,    "../log/newplayers" },
     { "rentgone",       ADMLVL_GRGOD,    "../log/rentgone" },
     { "errors",         ADMLVL_GRGOD,    "../log/errors" },
     { "godcmds",        ADMLVL_GRGOD,    "../log/godcmds" },
     { "syslog",         ADMLVL_GRGOD,    "../syslog" },
     { "crash",          ADMLVL_GRGOD,    "../syslog.CRASH" },
     { "\n", 0, "\n" }
};
...
ACMD(do_fix)
{
  ...
  if ((cmbrk) && !OBJ_FLAGGED(obj, ITEM_BROKEN)) {
...
void innate_add(struct char_data * ch, int num, int sktype, int uses, int maxuses)
{
...
*/
void add_innate_timer(struct char_data *ch, int spellnum, int sktype) 
{
  int timer = 6; /* number of ticks */
  int uses=1, maxuses=1;
  ...
  if (is_innate_ready(ch, spellnum)) {
    innate_add(ch, spellnum, sktype, uses, maxuses);
  } else {
    send_to_char(ch, "BUG!\r\n");
  }
}
...
ACMD(do_heal)
{
  ...
  if (!*arg) {
    send_to_char(ch, "You have to try and heal SOMEONE.\r\n");
    return;
  }
  ...
  if (GET_POS(vict) > POS_DEAD && GET_POS(vict) < POS_STUNNED) {
    ...
    if (roll_skill(ch, SKILL_HEAL) >= dc) {
      ...
      act("$n's wounds are stablized by $N!", TRUE, vict, 0, ch, TO_ROOM);
    } else {
...
/* Caculate the max uses per day of an innate ability */
void set_innate_max(struct char_data *ch, int innate, int sktype)
{

  /* Because we don't have a unified skill/spell/feat list yet...*/
  if (sktype == SKTYPE_FEAT) {
    switch (innate) {
      default:
        break;
    }
