                   ...
                   char *str, size_t slen)
{
  ...
  if (!str_cmp(field, "strlen")) {                     /* strlen    */
    snprintf(str, slen, "%"SZT, strlen(vd->value));
    return TRUE;
    ...
    if (c) {
      ...
      *str = '\x1';
	  ...
	  snprintf(str, slen, "%d", GET_ALIGNMENT(c));
            ...
            snprintf(str, slen, "%d", compute_armor_class(c, NULL));
        ...
        case 'f':
          if (!str_cmp(field, "feat")) {
            if (subfield && *subfield) {
              int featnum = find_feat_num(subfield);
              if (HAS_FEAT(c, featnum)) {
                strcpy(str, "1");
              } else {
                strcpy(str, "0");
              }
...
/* see note in dg_scripts.h */
            ...
            if (RIDING(c)) {
              snprintf(str, slen, "%c%d", UID_CHAR, GET_ID(RIDING(c)));
            } else {
              *str = '\0';
            }
          }
            ...
            if (RIDDEN_BY(c)) {
              snprintf(str, slen, "%c%d", UID_CHAR, GET_ID(RIDDEN_BY(c)));
            } else {
              *str = '\0';
            }
          }
