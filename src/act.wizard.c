...
room_rnum find_target_room(struct char_data *ch, char *rawroomstr)
{
  ...
  if ((GET_ADMLEVEL(ch) < ADMLVL_GRGOD) && ZONE_FLAGGED(rm->zone, ZONE_NOIMMORT)){
...
void do_stat_object(struct char_data *ch, struct obj_data *j)
{
  ...
  send_to_char(ch, "In room: %d (%s@n), ", GET_ROOM_VNUM(IN_ROOM(j)),
...
void do_stat_character(struct char_data *ch, struct char_data *k)
{
  ...
  send_to_char(ch, "L-Des: @[3]%s@n", k->long_descr ? k->long_descr : "<None>\r\n");
  ...
  send_to_char(ch, "Armor Class: [%d (%+dsz%+ddx%+dcl)], Damage: [%2d], Saving throws: [%d/%d/%d]\r\n",
	  compute_armor_class(k, NULL), get_size_bonus(get_size(k)),
          ability_mod_value(GET_DEX(k)), class_armor_bonus(k), GET_DAMAGE_MOD(k), GET_SAVE_MOD(k, 0),
          GET_SAVE_MOD(k, 1), GET_SAVE_MOD(k, 2));

  send_to_char(ch, "Accuracy: [%d total (%dbase%+dmod%+dstr%+dsz)]\r\n", 
          (GET_ACCURACY_BASE(k) + GET_ACCURACY_MOD(k) + ability_mod_value(GET_STR(k)) + get_size_bonus(get_size(k))),
          GET_ACCURACY_BASE(k), GET_ACCURACY_MOD(k), ability_mod_value(GET_STR(k)), get_size_bonus(get_size(k)));
  ...
  if (RIDING(k)) {
    send_to_char(ch, "Riding: %s\r\n", GET_NAME(RIDING(k)));
  }
  if (RIDDEN_BY(k)) {
    send_to_char(ch, "Ridden by: %s\r\n", GET_NAME(RIDDEN_BY(k)));
  }
  ...
  if (k->affectedv) {
    for (aff = k->affectedv; aff; aff = aff->next) {
      /* Hack */
      if (aff->sktype == SKTYPE_FEAT)
        send_to_char(ch, "FEAT: (%3d rounds) @c%-21s@n ", aff->duration + 1, feat_list[(aff->type)].name);
      else if (aff->sktype == SKTYPE_SKILL)
        send_to_char(ch, "SKILL: (%3d rounds) @c%-21s@n ", aff->duration + 1, skill_name(aff->type));
      else
        send_to_char(ch, "SPELL: (%3d rounds) @c%-21s@n ", aff->duration + 1, skill_name(aff->type));
...
ACMD(do_advance)
{
  ...
  if (GET_ADMLEVEL(ch) < GET_ADMLEVEL(victim)) {
    send_to_char(ch, "Probably not your best idea today...\r\n");
    return;
  }
		...
		char *val_arg)
{
  ...
  case 33:
    ...
    break;
  case 34:
    send_to_char(ch, "Zizazat doesn't believe you should do this. Use advance instead!\r\n");
    return (0);
...
#define PLIST_FORMAT \
  "players [minlev[-maxlev]] [-n name] [-d days] [-h hours] [-i] [-m]"
...
ACMD(do_players)
{
  ...
  int low = 0, high = CONFIG_LEVEL_CAP, low_day = 0, high_day = 10000, low_hr = 0, high_hr = 24;
  ...
  len = 0;   
  len += snprintf(buf + len, sizeof(buf) - len, "@W[ Id] (Lvl) (Adm) Name          Last@n\r\n"
                  "@c--------------------------------------------------------@n\r\n");

  for (i = 0; i <= top_of_p_table; i++) {
    if (player_table[i].admlevel < low || player_table[i].admlevel > high)
      continue;
    ...
    len += snprintf(buf + len, sizeof(buf) - len, "[%3ld] (%3d)  (%d)  %c%-12s %s\r\n",
                    player_table[i].id, player_table[i].level, player_table[i].admlevel,
...
ACMD(do_raise)
{
  ...
  send_to_char(ch, "@yYou return %s from the @Bspirit@y world, to the world of the living!@n\r\n", GET_NAME(vict));
  send_to_char(vict, "@yYour @Bspirit@y has been returned to the world of the living by %s!@n\r\n", GET_NAME(ch));
...
#define MAX_ACCURACY_MOD_ALLOWED MAX(GET_LEVEL(mob)/3, 2)
...
struct zcheck_armor {
  ...
  {ITEM_WEAR_WINGS,	10,	"Wing item"},
  {ITEM_WEAR_MASK,	10,	"Mask item"}
};
...
struct zcheck_affs {
  ...
  {APPLY_UNUSED0,	0,	0,	"unused"},
  {APPLY_UNUSED1,	0,	0,	"unused"},
  {APPLY_UNUSED2,	0,	0,	"unused"},
 ...
 /************** Check mobs *****************/
  ...
  for (i=0; i<top_of_mobt;i++) {                   
      if (real_zone_by_thing(mob_index[i].vnum) == zrnum) {  /*is mob in this zone?*/
        ...
        if (GET_ACCURACY_MOD(mob)>MAX_ACCURACY_MOD_ALLOWED && (found=1))
          len += snprintf(buf + len, sizeof(buf) - len,
                          "- Accuracy mod of %d is too high (limit: %d)\r\n",
                          GET_ACCURACY_MOD(mob), MAX_ACCURACY_MOD_ALLOWED);
  ...
  for (i=0; i<top_of_objt; i++) {
    if (real_zone_by_thing(obj_index[i].vnum) == zrnum) { /*is object in this zone?*/
     ...
     for (todam=0, tohit=0, j=0;j<MAX_OBJ_AFFECT;j++) {
       if (obj->affected[j].location == APPLY_ACCURACY)
...
struct obj_data *find_in_container(struct obj_data *container, char *query)
{
  ...
  if (!container->contains)
    return NULL;
  for (i = container->contains; i ; i = i->next_content) {
    if (isname(query, i->name))
      return i;
    if (i->contains) {
      /* look in this container too */
      struct obj_data *tmp = find_in_container(i, query);
      /* did we find it ? */
      if (tmp)
        return tmp;
    }
...
/* this is a much needed utility function. */
struct obj_data *get_object_on_character(struct char_data *ch, struct char_data *vict, char *query)
{
  int i;
  struct obj_data *obj, *it;

  for (i = 0; i < NUM_WEARS; i++) {
    if (GET_EQ(vict, i) &&  CAN_SEE_OBJ(ch, GET_EQ(vict, i))) {
      /* have we got it ? */
      if (isname(query, GET_EQ(vict, i)->name))
        return GET_EQ(vict, i);

      /* perhaps it's inside this object? */
      obj = find_in_container(GET_EQ(vict, i), query);
      if (obj)
        return obj;
    }
  }
  /* ok, so it wasn't worn, or in worn items. Check inventory */
  for (it = vict->carrying; it; it = it->next_content) {
    if (CAN_SEE_OBJ(ch, it)) {
      if (isname(query, it->name))
        return it;

      /* perhaps it's inside this object somewhere? */
      obj = find_in_container(it, query);
      if (obj)
        return obj;
    }
  }
  /* nope, not on this bloke.. */
  return NULL;
}
